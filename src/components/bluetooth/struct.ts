					...
					case ETypeValue.TYPE_STRUCT:
						if (value[2] > 0) {
							for (let i = 0; i < value[2]; i++)
								this.offset = new Struct(value[1]).decode(buffer, data[key][i], this.offset);
						} else this.offset = new Struct(value[1]).decode(buffer, data[key], this.offset);
				}
				...
				if (value[0] != ETypeValue.TYPE_BIT) {
					...
					switch (value[0] as ETypeValue) {
						...
						case ETypeValue.TYPE_STRUCT:
							if (value[2] > 0) {
								for (let i = 0; i < value[2]; i++)
									this.offset = new Struct(value[1]).encode(buffer, data[key][i], this.offset);
							} else this.offset = new Struct(value[1]).encode(buffer, data[key], this.offset);
					}
